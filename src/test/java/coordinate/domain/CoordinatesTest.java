...
class CoordinatesTest {
    ...
    void coordinates_SameObject_Test() {
        ...
        assertThat(coordinates).isEqualTo(new Coordinates(List.of("1,2", "2,3")));
    }

    @ParameterizedTest
    @CsvSource(value = {"1,2:True", "2,1:False", "2,3:True", "3,2:False"}, delimiter = ':')
    @DisplayName("좌표를 포함하는지 확인하는 테스트")
    void coordinates_isContainCoordinate_Test(String coordinate, boolean expect) {
        Coordinates coordinates = new Coordinates(List.of("1,2", "2,3"));
        assertThat(coordinates.isContainCoordinate(new Coordinate(coordinate))).isEqualTo(expect);
    }
